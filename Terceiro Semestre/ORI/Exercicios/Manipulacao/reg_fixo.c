...
void mostrarRegistro(pessoa,int *nros);   // Mostra o registro na tela.
...
void ler(void)
{
   ...
   while(recuperarRegistro(buffer, arquivo))   //lendo registro inteiro do arquivo
   {
      ...
      mostrarRegistro(umaPessoa,&nros);          //mostrar Registro na Tela
      printf("Pressiona [ENTER] para mostrar o proximo registro\n\n");
      if(!condicao)
         getchar();
...
void mostrarRegistro(pessoa umaPessoa,int *nros)
{
   printf("Registro %d:\n",*nros);
...
{
   char bufferAux[tamanho];  //buffer utilizado para ler um campo do teclado
